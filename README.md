...
- 단일 입력란과 명시적 다운로드 큐
- `Ctrl+V`/`Shift+Insert`로 붙여넣은 URL 자동 큐 추가
- 큐 항목 삭제, 실패 항목 재시도, 실행 중 추가
- 앱 종료 후 미완료 큐 자동 복원 및 재개
...
- 실행 파일 옆 `settings.ini`에 저장 위치 및 다운로드 설정(원본 다운로드, arcatmp 삭제 여부) 저장
- 실행 파일 옆 `queue.json`에 미완료 큐 저장
